...
async function startSocketStream(io) {
    ...
    try {
      ...
      socket.on('open', async () => {
        ...
        if (isMarketOpen()) {
          ...
          console.log(
            `📊 Found ${symbols.length} symbols to subscribe to:`,
            symbols
          );
          ...
          if (msg.type === 'trade' && Array.isArray(msg.data)) {
            ...
            for (const json of msg.data) {
              if (json.p) {
                ...
                const { ask, bid } = await applyBrokerSpread(
                  Number(json.p),
                  json.s
                );  //it return promiss get symbol fromd atabase and calculate ask and bid
  ...
  return {
    ...
    refreshSubscriptions: () =>
      symbolSubscriptionManager.refreshSubscriptions(),
    getSubscriptionStatus: () =>
      symbolSubscriptionManager.getSubscriptionStatus(),
    subscribeToSymbol: (symbol) =>
      symbolSubscriptionManager.subscribeToSymbol(symbol),
    unsubscribeFromSymbol: (symbol) =>
      symbolSubscriptionManager.unsubscribeFromSymbol(symbol),
  };
}
